...
const QuestionInner: FC<QuestionInnerProps> = ({ question, afterCommit }) => {
  const { Option, handleChange, handleCommit } = useQuestionFunc(
    question.isMultiple,
  )
  const handleClick = () => {
    handleCommit()
  ...
  return (
    <div className="flex flex-col  w-screen h-screen">
      ...
      <Option.Group className="m-4" onChange={handleChange}>
        <Space className="w-full" direction="vertical">
          {question.options.map(option => (
            <label key={option}>
              <Card bodyStyle={{ padding: 8 }} className="w-full">
      ...
      <div className="flex justify-center">
        <Button className="mx-4" type="primary" onClick={handleClick}>
          随机下一题
        </Button>
        <Button
          className="mx-4"
